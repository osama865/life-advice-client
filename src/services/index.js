...
// TODO EXPOSE ONLY TWO APIs, one is localhost:4000 as a dev server, the second is the proxy server
// TODO Refactor this shitty code
// todo put limitations on the amount of fetched data

const proxy =
    "https://cors-proxy4.p.rapidapi.com/?url=https%3A%2F%2Flife-advise-server.herokuapp.com%2F";
const prox2 =
    "https://http-cors-proxy.p.rapidapi.com/https://life-advise-server.herokuapp.com/multiple";
...
async function unsubscribeNotifications(data = {}) {
    ...
    let res = await fetch(dev + "unsubscribe", options)
    ...
    navigator.serviceWorker.ready.then(function (registration) {
        registration.pushManager.getSubscription().then((subscription) => {
            ...
            unsubscribeNotifications(subscription)
                .then((result) => {
                    console.log(
                        " the subscription data has been removed from the server. ",
                        result
                    );
                ...
                .catch((err) => {
                    console.error(
                        err,
                        "happend when removing subscription data from server"
                    );
                });
        });
    });
...
export async function fetchMultiple(skip = 0) {
    ...
    console.log("fetch multiple", dev + "multiple");

    console.log("fetch multiple with options", opt);
    const options = {
        method: "POST",
        headers: {
            "content-type": "application/json",
    ...
    let res = await fetch(dev + "multiple", options)
        .then((response) => response.clone().json())
        .then((response) => response)
        .catch((err) => console.error(err));
...
async function fetchData(route, options) {
    console.log("fetch request", `${dev}${route}`);
    let res = await fetch(`${dev}${route}`, options)
        ...
        .then((response) => response)
        .catch((err) => {
            console.error("error occured in the fetch step", err);
        });
    return res;
}
...
export const test = {
    _id: "39jd",
    text: "fgn our absence.",
    author: "Salman Rushdie",
    date: "2021-12-30T08:02:42.027Z",
    index: 5,
    language: "ar",
};
