...
export default function Translate({ changeText, setTo }) {
    ...
    let list = Object.values(languages)
    let shortcuts = Object.keys(languages)
        ...
        <>
            ...
            <div className="box">
                <select onChange={(e) => {
                    ...
                    {
                        // sometimes, i scared with my IQ!
                        list.map((lang, index) => (<option value={shortcuts[index]} key={index}>{lang.name}</option>))
                    }
                </select>
            </div>
        </>
